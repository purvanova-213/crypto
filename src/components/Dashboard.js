    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-content">
        ...
        <div className="market-trends">
          ...
          <p>
            Market Cap:{" "}
            {marketTrends.market_cap &&
              `$${marketTrends.market_cap.usd.toLocaleString()}`}
          </p>
          <p>
            24h Total Volume:{" "}
            {marketTrends.total_volume &&
              `$${marketTrends.total_volume.usd.toLocaleString()}`}
          </p>
      ...
      <div className="trading-panel">
        ...
        >
          ...
          {cryptoPrices.map((crypto) => (
            <option key={crypto.id} value={crypto.currency.toLowerCase()}>
              {crypto.currency}
            </option>
          ))}
        </select>
        ...
        <ul>
          {transactionHistory.map((transaction) => (
            <li key={transaction.id}>
              {transaction.date}: Bought {transaction.amount}{" "}
              {transaction.crypto} at ${transaction.price} each.
            </li>
          ))}
        </ul>
      </div>
